...
\t{epoll\_ctl}, and a call to \t{epoll\_wait} returns the ready descriptors. In
the event loop model, all blocking I/O occurs in a single loop which 

It's important that the only blocking operations happen in the main loop

Event loops are a good way to obtain this 
to get the level of concurrency that you need

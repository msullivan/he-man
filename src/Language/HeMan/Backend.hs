...
-- TODO merge simplify into this pass
optimizeTail (If e c a) = do
  e' <- optimizeExpr e
  if e' == Front.NumLit 0 then optimizeTail a else
    if constantExpr e' then optimizeTail c else do
      c' <- optimizeTail c
      a' <- optimizeTail a
      return (If e' c' a')
optimizeTail t = return t

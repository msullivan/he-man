...
spawning is somewhat more interesting.
% and maybe a bit novel? I dunno.
For foreign function calls, our main concern was being able to write a
single wrapper for call that handles a varying number of typed
arguments. To this end, we created an \t{ArgPacket} type class that
requires the ability to convert a packet into a list of \t{IExpr} and
to construct an \t{ArgPacket} from a list of variable names. Instances
of \t{ArgPacket} are provided for packets of $0$ to $5$ \t{Expr}
arguments. Now we can parameterize a \t{Call} wrapper over an
\t{ArgPacket} to provide a \t{Expr} based interface.

For thread declarations, we can produce a similar \t{ArgDecl} class
and associated instances that can convert a packet of \t{Type}s into a
list of \t{IType}s.

We want more than this, however: we want to ensure that both the code
for a declared thread uses its arguments properly at the declared
types code and that spawning threads provides arguments of the correct
types. To do this, we need some way to extract all of the phantom
types of the \t{Expr}s and \t{Type}s in \t{ArgPacket}s and
\t{ArgDecl}s
...
instance ArgPacket (Expr a, Expr b) (a, b) where
  toIExprList (E x1, E x2) = [x1, x2]
  makeVars [x1, x2] = (E (Var x1), E (Var x2))
-- ...

instance ArgDecls (Type a) [a] where
  toITypeList t1 = [toIType t1]
instance ArgDecls (Type a, Type b) (a, b) where
...
declare_thread :: (ArgPacket a c, ArgDecls b c) =>
                  ...
                  ThreadCode c
declare_thread = ...
spawn :: ArgPacket a b => ThreadCode b -> a ->
                          Prog ()
spawn = ...
call :: ArgPacket a b => Prim -> Type c -> a ->
                         Prog (Expr c)
call = ...
